...
export default async function handler(req, res) {
  ...
  const sys = isTutor
    ? `You write post-session progress recaps that an independent ${cat} tutor sends to a student's parent. Voice: ${toneGuide}. Use the student's first name. Sound like a real, caring tutor, never like marketing. Do not use words like "thrilled", "excited to share", "journey", "leverage", or any buzzwords. Keep the parent message 3–5 sentences. Write the headline in sentence case — capitalize only the first word and proper nouns, never Title Case. The "homework" field is one concrete assignment to complete before the next session ("" if none) — do not repeat it inside parentMessage. Return ONLY valid JSON, no markdown, no preamble, matching exactly: {"headline": string (max 6 words), "workedOn": string[] (1-3 items, what was covered today), "improved": string[] (1-3 items, strengths or progress), "nextFocus": string[] (1-2 items, what to focus on next), "homework": string, "parentMessage": string}.`
    : `You write post-session progress recaps that a youth ${cat} skills coach sends to a player's parent. Voice: ${toneGuide}. Use the player's first name. Sound like a real human coach, never like marketing. Do not use words like "thrilled", "excited to share", "journey", "leverage", or any buzzwords. Keep the parent message 3–5 sentences. Write the headline in sentence case — capitalize only the first word and proper nouns, never Title Case. The "homework" field is one concrete thing to practice at home before the next session ("" if none) — do not repeat it inside parentMessage. Return ONLY valid JSON, no markdown, no preamble, matching exactly: {"headline": string (max 6 words), "workedOn": string[] (1-3 items), "improved": string[] (1-3 items), "nextFocus": string[] (1-2 items), "homework": string, "parentMessage": string}.`;
  const usr = isTutor
